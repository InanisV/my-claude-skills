...
name: dev-planner
description: |
  🚀 超级开发规划师 —— 问题验证 + 对话式需求收集 + Agent 团队并行开发 + MCP 工具全程调用 + 开发复盘。

  触发时机：用户说"帮我开发"、"我想做一个项目/网站/APP/系统/工具/脚本/Bot"、"plan模式"、
  "对话式开发"、"生成开发文档"、"我有个想法想实现"，或任何描述了软件/工具/系统需求的请求。
  即使需求模糊如"我想做个东西"也要触发。核心能力：(1) 先验证问题再动手，
  (2) 引导小白用户通过选项对话完善需求，(3) 生成企业级超详细开发文档，
  (4) 编排 Agent 团队+MCP 工具并行开发，(5) 全程文档驱动、节点测试、自动提交，
  (6) 完成后结构化复盘总结经验。
---
...
> **AI 时代，把功能从 80% 做到 100% 的边际成本极低。所以永远追求完整交付，不留半成品。**
>
> 每个功能都写测试，每个接口都有文档，每个错误都有处理——因为补全它们只需要几分钟。
...
│  Phase 6  RETRO        开发复盘（总结经验、记录教训）           │
└─────────────────────────────────────────────────────────────┘
```

---

## Phase 0：问题验证（先想清楚再动手）

> 灵感来源：gstack /office-hours 的"六个逼问"方法论。
> 在写任何代码前，先验证这个东西值不值得做、方向对不对。
...
📊 2. 他们现在怎么解决这个问题？
   → 用 Excel？手动操作？用竞品？还是根本没解决？
   → 理解现状才能做出真正有价值的改进

🔥 3. 最核心的一个功能是什么？
   → 如果只能做一个功能，哪个功能能让用户说"这个好用"？
   → 先把这个做到极致，其他的后面再加
...
回答这些问题（能答几个答几个），或者说"跳过，直接开始规划" ⏭️
```
...
> 灵感来源：gstack /careful + /freeze + /guard 安全机制。

开发过程中，Agent 必须遵守以下安全规则：

```
⚠️ 破坏性操作警告 —— 执行以下命令前必须暂停并确认：
  • rm -rf（删除目录）     → 确认目标路径，排除误删
  • DROP TABLE / DROP DB  → 确认是否有备份
  • git push --force      → 确认是否会覆盖他人工作
  • git reset --hard      → 确认是否会丢失未提交代码

🔒 作用域锁定 —— 每个 Agent 只能修改自己负责的目录：
  • 后端 Agent → /src/server, /src/db, /src/api
  • 前端 Agent → /src/client, /src/components, /src/pages
  • 测试 Agent → /tests, /e2e
  • 跨目录修改 → 需主控 Agent 审批

🛡️ 爆炸半径检查 —— 单次修改超过 5 个文件时自动暂停，
   向用户展示变更范围并请求确认。
```
...
## Phase 5.5：遇到 Bug 时的调查协议

> 灵感来源：gstack /investigate 的系统化根因分析方法论。
> 核心铁律：**没有找到根因之前，禁止盲目修复。**

当开发过程中出现 Bug 或测试失败时，启动以下流程：
...
> 灵感来源：gstack /retro 的结构化回顾方法论。
> 每次开发完成后，花 2 分钟复盘，积累的经验比代码更有价值。
