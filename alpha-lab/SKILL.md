...
**Phase 2（防御期）假设方向**：🔴 优先创新（因子轮动、新信号、条件化选币），
  其次才是参数调优（止损逻辑、风控模块、震荡期仓位缩减）
...
判定标准（需同时满足至少 3 条）：
```
- 连续 15+ 个实验（含跳出局部最优的大胆尝试）score 提升 < 0.2%
- 已经尝试过至少 3 种不同的结构性改动方向（不只是参数微调）
- 当前最佳 score 在所有三个 Phase 维度上都没有明显短板
- 简洁性审计通过：删除任何模块都会导致 score 下降
- 已触发过至少 1 次"跳出局部最优"机制且未找到更好的解
```

达到此状态后，执行"研究终止交付流程"（见下方），宣布找到当前架构下的全局最优。
